...
<ul>
	<c:forEach var="project" items="${category.projects}">
		<li><a href="<c:url value="/project/${project.id}" />">${project.name}</a>
			<br>Gathered budget: ${project.gatheredBudget} <br>Required
			budget: ${project.requiredSum} <br>Days to go:
			${project.remainingDays} <br>Description: ${project.description}</li>
	</c:forEach>
</ul>
